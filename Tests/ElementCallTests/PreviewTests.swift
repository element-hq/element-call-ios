...
// publicly, and the harness below needs all four to apply per-preview precision. Adapted from
// Compound's harness, which does the same.
...
struct PreviewTests {
    ...
    private let simulatorDevice: String? = SnapshotEnvironment.simulatorModelIdentifier
    ...
    private func checkEnvironments() {
        if let simulatorDevice {
            ...
            guard deviceModel?.contains(simulatorDevice) ?? false else {
                fatalError("\(deviceModel ?? "Unknown") is the wrong simulator. Create and use \(SnapshotEnvironment.simulatorName) for these tests.")
            }
        }
        ...
        // The locale lands in every snapshot file name, so an unpinned one makes the images
        // machine-specific and 42 of them fail at once without saying why. element-x-ios pins this in
        // a test plan; a package has no test plan, so it comes from the command line.
        let language = Locale.current.language.languageCode?.identifier
        let region = Locale.current.language.region?.identifier
        guard language == SnapshotEnvironment.requiredLanguage, region == SnapshotEnvironment.requiredRegion else {
            fatalError("""
            Snapshots are recorded in \(SnapshotEnvironment.requiredLanguage)-\(SnapshotEnvironment.requiredRegion), \
            but this run is \(language ?? "??")-\(region ?? "??"). Add the following to the xcodebuild command:
              -testLanguage \(SnapshotEnvironment.requiredLanguage) -testRegion \(SnapshotEnvironment.requiredRegion)
            """)
        }
    }

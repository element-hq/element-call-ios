...
public nonisolated protocol ElementCallMatrixTransport: AnyObject, Sendable {
    ...
    /// Which media transports the homeserver offers, in its order of preference.
    ///
    /// Discovery is a Matrix client concern, not ours: it means reading the MSC4143 endpoint and
    /// falling back to `rtc_foci` in the well-known, with caching. The Rust SDK already does exactly
    /// that in `Client::discover_rtc_transports`, but over FFI it exposes only booleans, so a host
    /// has to reimplement it by hand today. This one method is what becomes a single SDK call when
    /// the bindings expose the list, and none of that plumbing belongs in this package meanwhile.
    ///
    /// We pick from what comes back, because only we know which transports we can actually drive.
    ///
    /// Transports are homeserver-wide, not room-scoped. The room is passed anyway because a host
    /// may need one to route the request: the SDK's widget driver serves this as the MSC4515
    /// `org.matrix.msc4515.get_rtc_transports` action, and a driver is opened per room. A host with
    /// a direct route should ignore it.
    ///
